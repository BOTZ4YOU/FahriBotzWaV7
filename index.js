...
const bugtrol = { 
	    ...
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
						...
						"message": `${creator}`,
						"orderTitle": `${creator}`,
						"sellerJid": "62857726746954@s.whatsapp.net",
...
let _votes = JSON.parse(fs.readFileSync(`./lib/vote/${from}.json`))  
let fil = vote.map(v => v.participant)
let id_vote = sender ? sender : '62857726746954@s.whatsapp.net'
if(fil.includes(id_vote)) {
  ...
  //FUNCTION
    ...
    if (isGroup && !mek.key.fromMe && isAntiLink && !isGroupAdmins && !isOwner && !isCreator && isBotGroupAdmins){
            if (chatxs.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                reply2(`「 G R O U P  L I N K  D E T E C T O R 」\n\nKamu telah terdeteksi mengirimkan link grup, maaf kamu akan di kick`)
     ...
     if (isGroup && isAntivirtex && !mek.key.fromMe && !isOwner) {
      if (budy.length > 1000) {
        ...
        reply2("「 A N T I V I R T E X  D E T E C T E D 」 \n\n Kamu Terdeteksi  telah mengirimkan Virtex,Maaf kamu Akan Dikick");
        alpha.groupRemove(from, [sender]);
      }
    }
    if (isGroup && !isCmd) {
